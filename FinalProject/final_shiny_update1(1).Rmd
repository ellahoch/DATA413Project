...
server <- function(input, output, session) {
  observe({
    updateSelectInput(session, "dataSelect", 
                      choices = c("Pre-2008 Annual Reports", "Post-2008 Annual Reports"))
  })
  
  dataset <- reactive({
    if(input$dataSelect == "Annual reports"){
      readr::read_csv("preannual-income_update.csv")
    } else{
      readr::read_csv("postannual-income_update.csv")
    }
    })
  ...
  updateSliderInput(
    ...
    max = 100
        ...
        updateSelectInput(session, "corrSelect1", choices = names(df)[-1])
